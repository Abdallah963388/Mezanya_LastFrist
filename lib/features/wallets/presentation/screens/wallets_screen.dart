              ...
              builder: (ctx, setSheet) => Container(
                ...
                child: Column(
                  children: [
                    // ── Hero card ──────────────────────────────────────────
                    GestureDetector(
                      onTap: () => setSheet(() => expanded = !expanded),
                        ...
                        child: Column(
                          children: [
                            Row(
                              children: [
                                Container(
                                  width: expanded ? 70 : 58,
                                  height: expanded ? 70 : 58,
                                  decoration: BoxDecoration(
                                    color:
                                ...
                                // Actions
                                _iconAction(Icons.settings_outlined,
                                    onTap: () {
                            ...
                            if (expanded) ...[
                              ...
                              Row(
                                children: [
                                  ...
                                  Expanded(
                                      child: _glassMetric(
                                          label: 'محجوز للحصالات',
                                          value: reserved.toStringAsFixed(2),
                                          accent: accent)),
                                ],
                              ),
                              const SizedBox(height: 14),
                              // ── Distribution: which jars ──
                    ...
                    child: AnimatedContainer(
                      ...
                      child: Column(
                        children: [
                          ...
                          // ── Expanded: metrics + wallet distribution ──
                          if (expanded) ...[
                            const SizedBox(height: 16),
                            Row(
                              children: [
...
class _TransactionTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // الاسم: نوع التحويل الخاص → اسم الفئة → الملاحظات → النوع
    final label = switch (transaction.transferType) {
      ...
      _ => txDisplayName(transaction, categories),
    };

    final subtitle = transaction.transferType == null ||
            !['jar-allocation', 'jar-allocation-cancel', 'wallet-to-wallet']
                .contains(transaction.transferType)
        ? txSubtitle(transaction)
        : null;
